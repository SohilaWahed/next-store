...
import { appearOrder } from "./_action/order.action";
      ...
      {orders.length > 0 ? (
        orders.map((order) => (
          <div key={order._id} className="border rounded-lg p-4 mb-6 shadow">
            <h2 className="text-lg font-semibold mb-2">Order {order.paymentMethodType} </h2>
            <p>
              <strong>Paid At:</strong>{" "}
              {order.paidAt ? new Date(order.paidAt).toLocaleString() : "Not paid yet"}
            </p>
            <p><strong>Total Price:</strong> {order.totalOrderPrice} EGP</p>
            <p><strong>Payment Method:</strong> {order.paymentMethodType}</p>
            <p><strong>Paid Status:</strong> {order.isPaid ? "Paid" : "Not Paid"}</p>
